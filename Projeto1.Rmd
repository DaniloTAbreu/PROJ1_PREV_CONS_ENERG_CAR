...
O problema de negócio já informa queé requerido um modelo de Machone Learning. No dataset temos a coluna "mean - Energy consumption [kWh/100 km]" que é a variável que queremos prever. Desta forma, iremos utilizar aprendizagem supervisionada.
...
#[5] "Engine power [KM]" - Força do motor                    
...
#coluna 22 boot_capac      1 dado missing
...
#coluna type_brak  1 dado missing
...
#O modelo 1 é muito complexo e pode estar em overfitting.

```


## Modelo de Random Forest para buscar as melhores variáveis
...
## Modelo de regressão linear múltipla com variáveis importantes obtidas do Decision Random Forest


```{r}
modelo_v6 <- lm(ener_consump ~ max_load_capac+perm_gross_weig+wheelbase+
                  length+tire_size+eng_power, treino)
summary(modelo_v6) # R2_v7 = 0.8741
#até o momento o modelo_v7 é o melhor
...
## Modelo de Random Forest com variáveis importantes obtidas do Decision Random Forest
...
```
## Modelo de Boosted Decision Tree Regression
```{r}
...
#data:  modelo_v7$residuals
#W = 0.98523, p-value = 0.8632
...
#considerando dataset pequeno e modelo somente com 3 variáveis preditivas, creio ser aceitável 
#taxa de acertos de previsão de consumo de energia de carros elétricos na faixa de 78%
